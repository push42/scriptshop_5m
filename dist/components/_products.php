...
<div class="container mx-auto px-4 pt-8 mb-8">
    <div class="flex justify-center items-center space-x-4">
        ...
        <select id="packageFilter" onchange="filterProducts()" class="px-4 py-2 rounded-lg border-2 bg-zinc-950 text-gray-200 border-zinc-500">
            ...
            <option value="av-package">AV Package</option>
            <!-- Add other packages as options -->
        </select>
    </div>
</div>
    ...
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            <div class="product-card bg-dark-theme rounded-lg overflow-hidden shadow-lg" data-package="<?php echo $product['package']; ?>" data-tags="<?php echo implode(', ', $product['tags']); ?>">
                ...
                <!-- Proof Button -->
                <button onclick="openModal('<?php echo $product['productid']; ?>')" class="bg-purple-900 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded w-full mt-4">
                    Proof
                </button>
            </div>
            <!-- Modal for product proof -->
            <div id="modal-<?php echo $product['productid']; ?>" class="modal fixed inset-0 bg-black bg-opacity-75 overflow-y-auto h-full w-full hidden-modal" style="backdrop-filter: blur(10px); z-index: 50;">
                <div class="relative top-1/4 mx-auto p-5 border w-1/2 shadow-lg rounded-md bg-white">
                <!-- Close Button -->
                <button onclick="closeModal('<?php echo $product['productid']; ?>')" class="absolute top-0 right-0 mt-4 mr-4 bg-gray-500 text-white rounded-md px-3 py-1 text-xs font-bold focus:outline-none">X</button>
                    <!-- Content for Modal -->
                    <div class="mt-4 text-center">
                        <h3 class="text-xl font-bold text-gray-800 mb-4">Proof of Ownership</h3>
                        <div class="mb-4">
                            <img src="./assets/images/products/proofs/proof-<?php echo $product['productid']; ?>.png" alt="Proof Image" class="mx-auto w-3/4 h-auto object-cover rounded-lg">
                        </div>
                        <div class="px-6">
                            <p class="text-gray-600 mb-4">Proof of purchase and authenticity for the "<?php echo $product['name']; ?>".</p>
                        </div>
                    </div>
                </div>
            </div>
